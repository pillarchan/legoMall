  ...
  $.fn.extend({
    tyslide: function(opts) {
      ...
      $(this).each(function(index, item) {
        ...
        $(item)
          .find('.innerwrapper')
          .css({ left: -boxwidth, height: boxheight }); //修改innerwrapper初始位置的坐标
        $(item)
          .find('.innerwrapper>li')
          .css({ width: boxwidth, height: boxheight, padding: '0', display: 'block' }); //修改li的宽度和高度
        $(item)
          .find('img')
          .css({ width: boxwidth, height: boxheight }); //修改图片的宽度和高度

        $(item)
          .find('.btnleft')
          .css({ opacity: '0.1' }); //设置按钮的透明度
        $(item)
          .find('.btnright')
          .css({ opacity: '0.1' }); //设置按钮的透明度
        ...
        $(item)
          .find('.controls>li')
          .css({ 'background-color': prams.controlsColor });
        //默认对第一个设置为当前按钮的颜色
        $(item)
          .find('.controls>li')
          .eq(0)
          .css({ 'background-color': prams.controlsCurrentColor });

        var lifirst = $(item)
          .find('.innerwrapper>li:first')
          .clone(); //克隆第一个li
        var lilast = $(item)
          .find('.innerwrapper>li:last')
          .clone(); //克隆最后一个li
        ...
        function playppt() {
          ...
          var indextemp = k - 1;
          if (k == boxsount + 1) {
            //因为装图片的盒子有6个，控制器的盒子只有4个，当大盒子索引为5的时候  控制器的索引改为0
            indextemp = 0;
          }
          $(item)
            .find('.controls>li')
          ...
          $(item)
            ...
            .animate({ left: leftx }, function() {
              ...
              if (leftx == -boxwidth * (boxsount + 1)) {
                $(item)
                  .find('.innerwrapper')
                  .css({ left: -boxwidth + 'px' }); //将第二个盒子（索引=1）移动到当前的视窗里
              ...
              if (leftx == 0) {
                ...
                $(item)
                  .find('.innerwrapper')
                  .css({ left: posx + 'px' }); //将第5个盒子设置到当前的视窗
                //console.log("执行完毕");
        ...
        $(item).hover(
          ...
          function() {
            timer = setInterval(function() {
              //此处的timer前面就不要去加var了，因为前面已经定义了，再加就重复定义了，就会清理不到
              ++k;
