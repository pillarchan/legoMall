(function($) {
  ...
  window.magnifier = function(magnifierAttr) {
    // 合并参数
    var magnifier = $.extend({}, defaults, magnifierAttr);
    ...
    _this.eqImg = function() {
      ...
      function imgLoadEnd() {
        ...
        imgBox
          .empty()
          .append(
            '<img src="' + src + '" width="' + imgWidth + '" height="' + imgHieght + '" style="' + styleCss + '" />'
          );
        //重新计算移动盒子与图片的倍数
        ...
        _this.view
          .empty()
          .append(
            '<img src="' + src + '" width="' + imgWidth * multiple + '" height="' + imgHieght * multiple + '" />'
          );
        //偏移量
    ...
    _this.imgMove = function(_boole) {
      ...
      magnifier.index < 0
        ? (magnifier.index = 0)
        : magnifier.index > lineLenght - _deviation
        ? (magnifier.index = lineLenght - _deviation)
        : magnifier.index;
